...
public:
	...
	int open( const char* name, int w, int h, int frame, int sampler, int pernum, unsigned char *sps, int sps_size )
	{
		...
		sps[5], //sps[1] AVCProfileIndication
		sps[6], //sps[2] profile_compat
		sps[7], //sps[3] AVCLevelIndication
		...
		{
			printf("MP4SetTrackESConfiguration call failed.\n");
		}
...
class write_mp4{
	mp4v2_imp* m_mp4[MAX_GROUP*MAX_VENC];
	bool m_b_loop;
public:
	write_mp4()
	: m_b_loop( false )
	...
	void write_video(int chn, unsigned char * data, size_t size, int frametype, unsigned long long ts )
	{
		...
		if( m_mp4[chn] )
		{
			m_mp4[chn]->write_video( data, size, frametype == H264_IFRAME ? 1 : 0, ts );
		}
	...
	void write_audio( int chn, unsigned char* data, size_t size, int frametype, unsigned long long ts )
		...
		if( m_mp4[chn] )
		{
			m_mp4[chn]->write_audio( data + 7, size - 7, ts );
		}
	}
